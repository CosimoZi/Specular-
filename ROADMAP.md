...
## ✅ Phase 6 — 角色配置 + 团队 focus（已完成）

**目标**：单角色"伤害计算页"消失。`/characters` 成为配置 gallery，伤害展示移到 `/team` 并以"focus 角色"为视角，显示他在这队里收到了哪些 buff + 各技能的伤害。

### 已交付：
- `/characters` 分两段：已配置（按最近修改排序、彩色）+ 未配置（灰显）
- `isConfigured()` 检测：碰过 weapon / 圣遗物 / 命座 / UID 导入即算
- `/characters/:id` 纯配置编辑器 — 不再有伤害表，含一个跳转 `/team` 的 CTA
- 敌人 + 反应从 `CharacterConfig` 搬到 `TeamConfig`（更合逻辑：同一角色在不同队伍里反应不同）
- `useTeamConfig` Zustand 持久化（slots/focus/enemy/reaction/buffToggles）
- `/team` 新版：
  - 4 个 slot，picker 只列已配置角色
  - 每个 slot 单选 "Focus"
  - 敌人 + 反应在团队层面
  - **可用 buff 列表**：自动按队伍组成筛选，每条可开/关，命座/天赋等级不达标的会隐藏
  - **Focus 角色伤害展示**：最终面板 + 每技能 hit 的非暴击/暴击/期望 + 副词条边际收益
- `BuffSpec` 数据库：17 个标杆 buff（覆盖班尼特、万叶、法鲁扎、希诺宁、芙宁娜、申鹤、莫娜、钟离、裟罗、罗莎莉亚、玛薇卡、温迪、砂糖、五郎、闲云、卡齐娜、行秋 C6）
- buff 描述双语 + 数值近似（精确化是后续 TODO）

### 未做（已知 TODO，等用户决定优先级）
- 完成 129 个角色全部 buff 标注（巨量人工工作；目前 17/129）
- 角色定位标签（前台/后台/支援/治疗 等）
- 武器精炼效果应用（每把武器手工建模）
- 圣遗物 4pc 套装条件式效果（如 4 黄金剧团 4 层叠加 +X% 攻击力）
- buff 与角色 self 的细粒度（如 "scaledFromSourceAtk" 让班尼特给的 ATK 真正基于他自己的基础攻击）
- 月反应专用引擎（菲林斯等纳塔角色）
